...
import { Guest as GuestEntity } from './guest.entity';
...
export class GuestService {
    ...
    async findByUserId(userId: number) {
        return this.guestRepository
            .findOne({ where: { user: { id: userId } }, relations });
    }
    ...
    async incrementExecutionsCount(userId: number): Promise<UpdateResult> {
        return this.guestRepository.increment(
            { user: { id: userId } },
            'executionsCount',
            1
        );
    ...
    async decrementExecutionsCount(userId: number): Promise<UpdateResult> {
        return this.guestRepository.decrement(
            { user: { id: userId } },
            'executionsCount',
            1
        );
