...
export class QueueMessageService {
    ...
    public async sendQueueMessage(eventType: QueueMessageBody['type'], job: Job, queuePosition?: number) {
        ...
        if (queueCallbackUrl) {
            ...
            switch (eventType) {
                case 'UPDATE':
                    await Axios.post<unknown, unknown, QueueMessageBody>(queueCallbackUrl, {
                        ...baseQueueMessageBody,
                        type: eventType,
                        queuePosition,
                    });
