...
module.exports = function (element, bpmnFactory, options, translate) {
    ...
    if (
        !inputOutputHelper.isInputOutputSupported(element, insideConnector) ||
    ...
    append(entries, getOutputParameterEntries(element, bpmnFactory, parametersOptions, translate));
...
function getOutputParameterEntries(element, bpmnFactory, options, translate) {
    ...
    var outputParameters = params.map(function (param, index) {
        function onRemove() {
            ...
            if (
                inputOutput.get('outputParameters').length === 1 &&
                (inputOutput.get('inputParameters') || []).length === 0
            ) {
                if (!insideConnector) {
                    commands.push(
                        extensionElementsHelper.removeEntry(getBusinessObject(element), element, inputOutput),
                    );
                } else {
