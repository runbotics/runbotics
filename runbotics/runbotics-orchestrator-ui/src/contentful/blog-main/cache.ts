...
export function setBlogPostsCache(language:Language, cacheValue: BlogPost[]) {
    contentfulCache[language].set(`posts_${getCurrentDateString()}`, cacheValue);
}

export function getBlogCategoriesCache(language:Language) {
    return contentfulCache[language].get(`categories_${getCurrentDateString()}`) as Category[] | undefined;
}

export function setBlogCategoriesCache(language:Language, cacheValue: Category[]) {
    contentfulCache[language].set(`categories_${getCurrentDateString()}`, cacheValue);
}

export function getBlogTagsCache(language:Language) {
    return contentfulCache[language].get(`tags_${getCurrentDateString()}`) as Tag[] | undefined;
}
...
export function getBlogMainCache(language:Language): Awaited<ReturnType<typeof getMainPage>> | null {
    ...
    if (!contentfulCache[language].size || !isCacheUpToDate(language)) return null;
...
export function setSinglePostCache(language: Language,post: BlogPost) {
    contentfulCache[language].set(`${post.slug}_${getCurrentDateString()}`, post);
}

export function getSinglePostCache(language:Language, cacheKey: string) {
    return contentfulCache[language].get(`${cacheKey}_${getCurrentDateString()}`) as BlogPost | undefined;
}
