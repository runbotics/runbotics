import { BasicCredentialDto, CredentialTemplateNames } from '../Credential/Credential.types';
import { collectionColors } from '../CredentialsCollection/EditCredentialsCollection/CollectionColor/CollectionColor.types';

export const tempCredentialsInitial = [{
    id: 'jakies_id',
    name: 'Jakas Nazwa',
    tenantId: 'b7f9092f-5973-c781-08db-4d6e48f78e98',
    collectionId: 'kolekcja_asi',
    description: 'Jakies description',
    collectionColor: collectionColors.LIGHT_ORANGE.hex,
    createdBy: 'userId',
    createdOn:'2024-05-05',
    template: {id: 'custom', name: CredentialTemplateNames.CUSTOM, attributes: []},
    templateId: ''
}, {
    id: 'tymczasowe_id',
    name: 'Tymczasowa Nazwa',
    tenantId: 'b7f9092f-5973-c781-08db-4d6e48f78e98',
    collectionId: 'kolekcja_asi',
    description: 'Tymczasowe description',
    collectionColor: collectionColors.DARK_ORANGE.hex,
    createdBy: 'userId',
    createdOn:'2024-05-05',
    template: {id: 'custom', name: CredentialTemplateNames.CUSTOM, attributes: []},
    templateId: ''
}, {
    id: 'zmyslone_id',
    name: 'Zmyslona Nazwa',
    tenantId: 'b7f9092f-5973-c781-08db-4d6e48f78e98',
    collectionId: 'kolekcja_asi',
    description: 'Zmyslone description',
    collectionColor: collectionColors.LIGHT_GREEN.hex,
    createdBy: 'userId',
    createdOn:'2024-05-05',
    template: {id: 'custom', name: CredentialTemplateNames.CUSTOM, attributes: []},
    templateId: ''
}, {
    id: 'nowe_id',
    name: 'Nowa Nazwa',
    tenantId: 'b7f9092f-5973-c781-08db-4d6e48f78e98',
    collectionId: 'kolekcja_stasia',
    description: 'Jakies description',
    collectionColor: collectionColors.DARK_GREEN.hex,
    createdById: 'userId',
    createdAt:'2024-05-05',
    template: {id: 'custom', name: CredentialTemplateNames.CUSTOM, attributes: []},
    templateId: ''
}, {
    id: 'unikalne_id',
    name: 'Unikalna Nazwa',
    tenantId: 'b7f9092f-5973-c781-08db-4d6e48f78e98',
    collectionId: 'kolekcja_basi',
    description: 'Unikalne description',
    collectionColor: collectionColors.LIGHT_BLUE.hex,
    createdBy: 'userId',
    createdOn:'2024-05-05',
    template: {id: 'custom', name: CredentialTemplateNames.CUSTOM, attributes: []},
    templateId: ''
},
{
    id: 'ciemne_niebieskie_id',
    name: 'Ciemna Niebieska Nazwa',
    tenantId: 'b7f9092f-5973-c781-08db-4d6e48f78e98',
    collectionId: 'kolekcja_basi',
    description: 'Ciemne niebieska description',
    collectionColor: collectionColors.DARK_BLUE.hex,
    createdBy: 'userId',
    createdOn:'2024-05-05',
    template: {id: 'custom', name: CredentialTemplateNames.CUSTOM, attributes: []},
    templateId: ''
}, {
    id: 'szare_id',
    name: 'Szara Nazwa',
    tenantId: 'b7f9092f-5973-c781-08db-4d6e48f78e98',
    collectionId: 'kolekcja_basi',
    description: 'Szare description',
    collectionColor: collectionColors.LIGHT_GREY.hex,
    createdBy: 'userId',
    createdOn:'2024-05-05',
    template: {id: 'custom', name: CredentialTemplateNames.CUSTOM, attributes: []},
    templateId: ''
}, {
    id: 'ciemne_szare_id',
    name: 'Ciemna Szara Nazwa',
    tenantId: 'b7f9092f-5973-c781-08db-4d6e48f78e98',
    collectionId: 'kolekcja_ady',
    description: 'Ciemne szare description',
    collectionColor: collectionColors.DARK_GREY.hex,
    createdBy: 'userId',
    createdOn:'2024-05-05',
    template: {id: 'custom', name: CredentialTemplateNames.CUSTOM, attributes: []},
    templateId: ''
}];

export const tempCredentials: BasicCredentialDto[] = [
    {
        id: '30af0232-bd25-41f7-8c16-d401d0c342ef',
        name: 'Credential3',
        tenantId: 'b7f9092f-5973-c781-08db-4d6e48f78e98',
        description: null,
        createdAt: '2024-07-08T08:12:01.224Z',
        createdById: '3',
        updatedAt: '2024-07-08T08:12:01.224Z',
        updatedById: '3',
        templateId: '8e500566-5db2-4454-8df5-1ee0aa1e625e',
        collectionId: 'kolekcja_ady',
        collectionColor: collectionColors.DARK_GREY.hex,
        attributes: [
            {
                id: 'dd3d869b-42e2-42f3-acd9-8fa4695539ea',
                name: 'username',
                tenantId: 'b7f9092f-5973-c781-08db-4d6e48f78e98',
                description: null,
                masked: true,
                type: 'string',
                secretId: '835831be-3d34-4e94-ad1d-542555054bc3',
                createdAt: '2024-07-08T08:14:38.400Z',
                createdById: '3',
                updatedAt: '2024-07-08T08:32:50.697Z',
                updatedById: '3',
                credentialId: '30af0232-bd25-41f7-8c16-d401d0c342ef'
            },
            {
                id: '54610c6a-0c1d-45f6-a8a2-8231a2a36ee6',
                name: 'password',
                tenantId: 'b7f9092f-5973-c781-08db-4d6e48f78e98',
                description: null,
                masked: true,
                type: 'string',
                secretId: '5b5b48b8-1397-4b56-bcad-0a60e2be04ea',
                createdAt: '2024-07-08T08:54:21.816Z',
                createdById: '3',
                updatedAt: '2024-07-08T08:54:21.816Z',
                updatedById: '3',
                credentialId: '30af0232-bd25-41f7-8c16-d401d0c342ef'
            },
        ]
    },
    {
        id: '6381c6dd-031b-42a7-bc30-0dec69d32334',
        name: 'Credential1',
        tenantId: 'b7f9092f-5973-c781-08db-4d6e48f78e98',
        description: null,
        createdAt: '2024-07-08T09:55:50.303Z',
        createdById: '3',
        updatedAt: '2024-07-08T09:55:50.303Z',
        updatedById: '3',
        templateId: '375ba506-de65-40b2-a751-614912075a2b',
        collectionId: 'kolekcja_basi',
        collectionColor: collectionColors.LIGHT_GREEN.hex,
        attributes: []
    },
    {
        id: '123456dd-031b-42a7-bc30-0dec69d12345',
        name: 'Custom Credential',
        tenantId: 'b7f9092f-5973-c781-08db-4d6e48f78e98',
        description: null,
        createdAt: '2024-07-08T09:55:50.303Z',
        createdById: '3',
        updatedAt: '2024-07-08T09:55:50.303Z',
        updatedById: '3',
        templateId: '1v400222-5db2-4454-8df5-1ee0aa1e123d',
        collectionId: 'kolekcja_basi',
        collectionColor: collectionColors.LIGHT_GREEN.hex,
        attributes: []
    },
    {
        id: 'jakies_id',
        name: 'New one',
        tenantId: 'b7f9092f-5973-c781-08db-4d6e48f78e98',
        description: null,
        createdAt: '2024-07-08T09:55:50.303Z',
        createdById: '3',
        updatedAt: '2024-07-08T09:55:50.303Z',
        updatedById: '3',
        templateId: '1v400222-5db2-4454-8df5-1ee0aa1e123d',
        collectionId: 'kolekcja_basi',
        collectionColor: collectionColors.LIGHT_GREEN.hex,
        attributes: []
    }
];

export const getCredentials = () => tempCredentials;
