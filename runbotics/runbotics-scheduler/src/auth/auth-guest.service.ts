...
import { DEFAULT_TENANT_ID, Role } from 'runbotics-common';
...
export class AuthGuestService {
    ...
    async verifyGuestLimit(ip: string) {
        const quests = (await this.dataSource.manager.find(Guest)).filter(
            (guest) => bcrypt.compareSync(ip, guest.ipHash)
        );
        return quests.length === 0;
    }

    async createGuestUser(ip: string, langKey: string) {
        const newGuest = await this.dataSource.manager.transaction(
            async (transactionalEntityManager) => {
                const newGuestUser = new User();
                newGuestUser.langKey = langKey;
                newGuestUser.activated = true;
                newGuestUser.authorities = [
                    await transactionalEntityManager.findOne(Authority, {
                        where: {
                ...
                newGuestUser.tenantId = DEFAULT_TENANT_ID;
                ...
                const newGeneratedGuestUser =
                    await transactionalEntityManager.save(User, newGuestUser);
    ...
    async deleteOldGuests() {
        ...
        const guests = await this.dataSource.manager.find(Guest, {
            relations: { user: true },
        });
        await this.dataSource.manager.delete(User, {
            id: In(guests.map((guest) => guest.user.id)),
        });
    }
}
