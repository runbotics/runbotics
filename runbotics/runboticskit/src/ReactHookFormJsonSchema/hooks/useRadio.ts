...
import { UseRadioParameters, BasicInputReturnType, UseRadioReturnType, InputTypes } from './types';
...
const getItemInputId = (pointer: string, index: number, items: string[]): string => {
    return pointer + '-radio-input-' + (items[index] ? items[index] : '');
};

const getItemLabelId = (pointer: string, index: number, items: string[]): string => {
    return pointer + '-radio-label-' + (items[index] ? items[index] : '');
};

export const getRadioCustomFields = (baseInput: BasicInputReturnType): UseRadioReturnType => {
    ...
    return {
        ...
        getLabelProps: () => {
            ...
            labelProps.htmlFor = currentObject.title !== undefined ? currentObject.title : baseInput.pointer;
            return labelProps;
        },
