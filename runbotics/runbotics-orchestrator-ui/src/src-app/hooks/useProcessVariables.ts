...
const useProcessVariables = () => {
    ...
    const { passedInVariables } = useSelector(
        state => state.process.modeler);
    
    const getActionVariables = () => {
        const canvas = context?.modeler?.get('canvas');
        const rootElement = canvas.getRootElement();
        const processVariables = getProcessVariables(rootElement.businessObject);

        return processVariables;
    };

    const taggedActionVariables = getActionVariables().map(actionVariable => ({
        name: `#{${actionVariable.name}}`,
        tag: VariableTag.ActionAssigned
    }));


    const taggedGlobalVariables = globalVariables.map(item => ({
        name: `#{${item.name}}`,
