...
const Actions: FC<{ params: GridCellParams }> = (props) => {
    ...
    const hasAccessToEditAction = useFeatureKey([FeatureKey.EXTERNAL_ACTION_EDIT]);
    const hasAccessToDeleteAction = useFeatureKey([FeatureKey.EXTERNAL_ACTION_DELETE]);

    return (
        <>
            <If condition={hasAccessToEditAction}>
                <IconButton
                    onClick={() => {
            ...
            <If condition={hasAccessToDeleteAction}>
                <DeleteActionDialog
                    action={action}
                    open={showDelete}
