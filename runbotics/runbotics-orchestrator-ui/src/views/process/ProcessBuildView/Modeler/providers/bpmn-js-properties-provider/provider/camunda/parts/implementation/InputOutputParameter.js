...
module.exports = function (parameter, bpmnFactory, options, translate) {
    ...
    entries.push(
        entryFactory.validationAwareTextField(translate, {
            ...
            label: is(parameter, 'camunda:InputParameter')
                ? translate('Local Variable Name')
                : translate('Process Variable Name'),
            modelProperty: 'name',
    ...
    entries.push(
        entryFactory.autoSuggest(translate, {
    ...
    entries.push(
        entryFactory.table(translate, {
            ...
            addElement: function (element, node) {
                var newValue = createElement('camunda:Value', getParameter().definition, bpmnFactory, {
                    value: undefined,
                });
            ...
            removeElement: function (element, node, idx) {
                return cmdHelper.removeElementsFromList(element, getParameter().definition, 'items', null, [
                    getParameter().definition.items[idx],
                ]);
            },
    ...
    entries.push(
        entryFactory.table(translate, {
            ...
            addElement: function (element, node) {
                var newEntry = createElement('camunda:Entry', getParameter().definition, bpmnFactory, {
                    key: undefined,
                    value: undefined,
                });
            ...
            removeElement: function (element, node, idx) {
                return cmdHelper.removeElementsFromList(element, getParameter().definition, 'entries', null, [
                    getParameter().definition.entries[idx],
                ]);
            },
            ...
            editable: function (element, node, prop, idx) {
                ...
                return (
                    prop === 'key' ||
                    (!isMap(entry.definition) && !isList(entry.definition) && !isScript(entry.definition))
                );
            },
...
function isInsideExpression(value, index) {
    ...
    return openIndex > -1 && openIndex <= index && closeIndex > -1 && index < closeIndex;
}
...
function isInsideUnclosedExpression(value, index) {
    ...
    return openIndex > -1 && openIndex <= index;
}
