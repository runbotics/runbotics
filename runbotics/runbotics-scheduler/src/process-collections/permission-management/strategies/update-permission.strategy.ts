...
export class UpdatePermissionStrategy implements PermissionStrategy {
    ...
    async execute(): Promise<void> {
        ...
        await this.permissionRepository.revokeAccess(this.userId, this.collectionId);

        // 3. Zaktualizuj konkretne pola (tu akurat grant działa jako insert/update)
        await this.permissionRepository.grantAccess(this.userId, this.collectionId, this.newLevel);
