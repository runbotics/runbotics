...
module.exports = function (element, bpmnFactory, options, translate) {
    ...
    append(entries, getInputParameterEntries(element, bpmnFactory, parametersOptions, translate));
...
function getInputParameterEntries(element, bpmnFactory, options, translate) {
    ...
    var inputParameters = params.map(function (param, index) {
        function onRemove() {
            ...
            if (
                inputOutput.get('inputParameters').length === 1 &&
                (inputOutput.get('outputParameters') || []).length === 0
            ) {
                if (!insideConnector) {
                    commands.push(
