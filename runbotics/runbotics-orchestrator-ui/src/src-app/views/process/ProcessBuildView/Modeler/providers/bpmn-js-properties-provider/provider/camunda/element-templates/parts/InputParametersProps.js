    ...
    function renderInputParameter(id, templateProperty) {
        ...
        collapsibleEntry.get = function () {
            return assign(defaultGet(), {
                title: templateProperty.label ? translate(templateProperty.label) : templateProperty.binding.name,
            });
        };
        ...
        if (templateProperty.description) {
            parameterEntries.splice(
                1,
                0,
                createDescriptionEntry(templateProperty.description, id, collapsibleEntry.isOpen, translate),
            );
        }

        // (5) add parameter toggle
        parameterEntries.splice(
            templateProperty.description ? 2 : 1,
            0,
            entryFactory.toggleSwitch(translate, {
...
function createNewInputParameter(element, binding, bpmnFactory) {
    ...
    if (!extensionElements) {
        ...
        updates.push(cmdHelper.updateBusinessObject(element, bo, { extensionElements: extensionElements }));
    }
    ...
    updates.push(
        cmdHelper.addAndRemoveElementsFromList(element, inputOutput, 'inputParameters', null, [inputParameter], []),
    );
