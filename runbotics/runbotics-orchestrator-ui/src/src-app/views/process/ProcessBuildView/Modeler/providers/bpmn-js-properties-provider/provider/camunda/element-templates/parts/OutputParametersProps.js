...
module.exports = function (group, element, elementTemplates, bpmnFactory, translate) {
    ...
    function renderOutputParameter(id, templateProperty) {
        ...
        var collapsible = entryFactory.collapsible({
            ...
            cssClasses: ['bpp-collapsible--with-mapping', 'bpp-collapsible--with-template-out'],
            onToggle: onToggle,
            open: false,
            get: function () {
                return {
        ...
        if (templateProperty.description) {
            parameterEntries.push(createDescriptionEntry(templateProperty.description, id, isOpen, translate));
        }
...
function createNewOutputParameter(element, binding, bpmnFactory) {
    ...
    updates.push(
        cmdHelper.addAndRemoveElementsFromList(element, inputOutput, 'outputParameters', null, [outputParameter], []),
    );
