...
import { ErrorTypes, CustomValidators, CustomValidatorReturnValue } from './types';
...
import { getDateValidator } from './getDateValidator';

type GetCustomValidatorReturnType = Record<string, (value: string) => CustomValidatorReturnValue>;

function getCustomValidator(
    customValidators: CustomValidators,
    context: JSONSubSchemaInfo,
): GetCustomValidatorReturnType {
    return Object.keys(customValidators).reduce((acc: GetCustomValidatorReturnType, key: string) => {
...
export const getValidator = (context: JSONSubSchemaInfo, customValidators: CustomValidators): RegisterOptions => {
    ...
    const hasValidate = Object.keys(customValidators).length > 0 || JSONSchema.enum;
    ...
    switch (JSONSchema.type) {
        ...
        case 'string':
            if (JSONSchema.format == 'date-time' || JSONSchema.format == 'time') {
                return getDateValidator(JSONSchema, validator);
            } else {
