...
var customServices = [
    ...
    'environment.services.getCurrentDateFormated',
...
var suggestionsVariables = [
    'environment.variables',
    'environment.variables.content',
    'environment.variables.content.input',
];
var suggestions = [
    'false',
    'true',
...
var autoSuggestTextBox = function (translate, options, defaultParameters) {
    ...
    resource.suggestItems = function (element, entryNode, event) {
        ...
        if (currentWord && canSuggest(currentWord, editorNode, focusNode)) {
            ...
            forEach(suggestions, function (suggestion) {
                ...
                var suggestionLowerCase = suggestion && suggestion.toLowerCase();
                if (suggestionLowerCase.includes(valueLowerCase)) {
            ...
            // forEach(items, function(_item) {
            //   var item = 'environment.output.' + _item;
    ...
    function getSuggestionListPosition(listNode, container) {
        ...
        if (
            cursorBounds.top + SUGGESTION_LIST_BOX_THRESHOLD + listBounds.height >
            clientBounds.height + clientBounds.top
        ) {
            orientation = 'top-';
        } else {
            orientation = 'bottom-';
