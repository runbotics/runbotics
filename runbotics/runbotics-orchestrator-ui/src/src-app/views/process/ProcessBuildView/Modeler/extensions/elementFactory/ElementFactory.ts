...
export class BPMNElementFactory {
    ...
    createTaskType = (
        ...
        additionalShapeAttrs?: Record<string, any>
    ) => {
        const businessObject = this.bpmnFactory.create(`bpmn:${type}`);
        if (configuration.businessObject) {
            Object.entries(configuration.businessObject).forEach(
                ([key, value]) => {
        ...
        const createParameter = (
            ...
            if (
                ...
                return this.bpmnFactory.create(
                    `camunda:${destination}Parameter`,
                    {
                        name: parameter.name,
                ...
                return this.bpmnFactory.create(
                    `camunda:${destination}Parameter`,
                    {
                        ...
                        definition: this.bpmnFactory.create('camunda:Map', {
                            entries: Object.entries(parameterMap.map).map(
                                ([key, value]) =>
                ...
                return this.bpmnFactory.create(
                    `camunda:${destination}Parameter`,
                    {
                        ...
                        definition: this.bpmnFactory.create('camunda:List', {
                            items: parameterList.list.map(value =>
        ...
        this.commandStack.execute(
            'properties-panel.update-businessobject-list',
            {
                ...
                objectsToAdd: Object.values(configuration.inputParameters).map(
                    inputParameter =>
                        createParameter(
        ...
        this.commandStack.execute(
            'properties-panel.update-businessobject-list',
            {
                ...
                referencePropertyName: null,
                objectsToAdd: Object.values(configuration.outputParameters).map(
                    outputParameter =>
                        createParameter(
    ...
    public createSequenceFlow(payload: {
        source: any;
        target: any;
        waypoints: Position[];
        default?: boolean;
