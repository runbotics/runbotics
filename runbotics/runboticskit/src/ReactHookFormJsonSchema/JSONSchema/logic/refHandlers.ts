...
import { getSplitPointer, concatFormPointer, JSONSchemaRootPointer } from './pathUtils';
...
export const getSchemaFromRef = (
    ...
    if (isAbsoluteURI($ref)) {
        ...
        if (baseUrl.hash) {
            return getSchemaFromRef(baseUrl.hash, IDRecord, IDRecord[`${baseUrl.origin}${baseUrl.pathname}`]);
        }
    } else if (isURIFragmentPointer($ref) && schema) {
        return getSplitPointer($ref).reduce((currentSchema: JSONSchemaType, pointer: string) => {
            if (currentSchema) {
...
export const resolveRefs = (schema: JSONSchemaType, idMap: IDSchemaPair, usedRefs: string[]): JSONSchemaType => {
    ...
    return Object.keys(resolvedRefs).reduce((acc: JSONSchemaType, key: string) => {
        if (
