import { FieldError, FormProviderProps } from 'react-hook-form';
...
export const getError = (
    ...
    if (currentObject.type === 'array') {
        ...
        if (currentValues && Array.isArray(currentValues)) {
            const numberOfSelected = currentValues.filter((x) => x !== false).length || 0;
            ...
            } else if (currentObject.maxItems && numberOfSelected > currentObject.maxItems) {
                return {
    ...
    const retError: ErrorMessage = {
        message: typeof errors.message == 'string' ? errors.message : ErrorTypes.undefinedError,
        expected: undefined,
    };
