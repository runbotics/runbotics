...
export class ProcessCollectionService {
    ...
    async getProcessCollectionById(id: string, user: User): Promise<ProcessCollection> {
        ...
        const hasAccess = this.hasAccess(user, collection);
    ...
    async checkHasAccessToCollection(user: User, processCollectionId: string): Promise<boolean> {
        ...
        const hasAccessToParent = await this.checkHasAccessToCollection(user, processCollectionId);
    ...
    async hasAccess(user: User, processCollection: ProcessCollection): Promise<boolean> {
        return this.checkHasAccessToCollection(user, processCollection?.parent?.id) &&
            await this.processCollectionRepository.findOneOrFail({
                where: {
                    users: In([user]),
                    id: processCollection.id,
                },
            })
            .catch(() => false)
            .then(() => true);
   ...
   async countAvailableCollectionsById(collectionId: string, tenantId: string, userId: string): Promise<number> {
    ...
    async countAvailableCollectionsByIds(collectionIds: string[], tenantId: string, userId: string): Promise<number> {
        return this.processCollectionRepository.createQueryBuilder('pc')
            .leftJoin('pc.users', 'u')
            .where('pc.id IN (:...collectionIds)', { collectionIds })
            .andWhere('pc.tenant_id = :tenantId', { tenantId })
            .andWhere('pc.is_public = true OR pc.created_by_id = :userId OR u.id = :userId', { userId })
            .select('COUNT(DISTINCT pc.id)', 'count')
            .getCount();
    }

    async findAvailableChildrenCollections(parentId: string, tenantId: string, userId: string): Promise<ProcessCollection[]> {
        return this.processCollectionRepository.createQueryBuilder('pc')
            .leftJoin('pc.users', 'u')
            .where('pc.id IN (:...parent_id)', { parent_id: parentId })
            .andWhere('pc.tenant_id = :tenant_id', { tenant_id: tenantId })
            .andWhere('pc.is_public = true OR pc.created_by = :user_id OR u.id = :user_id', { user_id: userId })
            .distinct(true)
            .getMany();
    }

    async findAvailableRootCollections(tenantId: string, userId: string): Promise<ProcessCollection[]> {
        return this.processCollectionRepository.createQueryBuilder('pc')
            .leftJoin('pc.users', 'u')
    ...
    async findUserAccessibleById(collectionId: string, userId: string, tenantId: string): Promise<ProcessCollection[]> {
        return this.processCollectionRepository.createQueryBuilder('pc')
            .leftJoin('pc.users', 'u')
            .where('pc.id = :collectionId', { collectionId })
    ...
    async checkCollectionAvailability(collectionId: string, user: User) {
        ...
        const hasUserAllAccess = user.authorities.some((authority) => authority.featureKeys.some(fk => fk.name = FeatureKey.PROCESS_COLLECTION_ALL_ACCESS));
        ...
        const isCollectionAvailable = await this.countAvailableCollectionsById(collectionId, tenantId, String(user.id)) !== 0;
    ...
    async getRootCollections(user: User): Promise<ProcessCollection[]> {
        ...
        const rootAccessibleCollections = this.findAvailableRootCollections(tenantId, String(user.id));
    ...
    async getChildrenCollectionsByRoot(user: User): Promise<ProcessCollection[]> {
        ...
        const availableChildren = await this.findAvailableRootCollections(tenantId, String(user.id));
    ...
    async getChildrenCollectionsByParent(parentId: string, user: User): Promise<ProcessCollection[]> {
        ...
        const childrenAccessibleCollections = parentId
            ? this.findAvailableChildrenCollections(parentId, tenantId, String(user.id))
            : this.findAvailableRootCollections(tenantId, String(user.id));
    ...
    async getCollectionAllAncestors(collectionId: string, user: User): Promise<ProcessCollectionDto[]> {
        ...
        const mappedAncestors = this.mapToDto(ancestors);

        if (hasUserAllAccess) {
            return mappedAncestors;
        }

        const hasAccessToAllAncestors = await this.countAvailableCollectionsById(collectionId, tenantId, String(user.id)) === 1;
    ...
    async delete(id: string, user: User): Promise<void> {
        ...
        const toDelete = await this.processCollectionRepository.findOneOrFail({ where: { id, tenantId } }).catch(() => {
    ...
    async update(user: User, updateDto: UpdateProcessCollectionDto, id: string): Promise<ProcessCollection> {
        ...
        processCollection.parent = updateDto.parent as ProcessCollection;
        processCollection.users = updateDto.users as User[];

        this.checkCollectionAvailability(id, user);

        const parent = updateDto.parent?.id
            ? await this.processCollectionRepository.findOneOrFail({ where: { id: updateDto.parent.id }, relations: RELATIONS })
                .catch((error) => {
                    throw new NotFoundException(`Cannot update: Parent collection with id ${updateDto.parent.id} could not be found`, error);
                })
            : null;
    ...
    async create(user: User, createDto: CreateProcessCollectionDto): Promise<ProcessCollection> {
        ...
        const parent = createDto.parent?.id
            ? await this.processCollectionRepository.findOneOrFail({ where: { id: createDto.parent.id }, relations: RELATIONS })
                .catch((error) => {
                    throw new NotFoundException(`Cannot create: Parent collection with id ${createDto.parent.id} could not be found`, error);
                })
            : null;
        processCollection.parent = parent;

        const hasParentAccess = await this.checkHasAccessToCollection(user, createDto.parent.id);
    ...
    async getAllPage(user: User, specs: Specs<ProcessCollection>, paging: Paging): Promise<Page<ProcessCollection>> {
        const options: FindManyOptions<ProcessCollection> = {
            ...paging,
            ...specs,
        ...
        const page = await getPage(this.processCollectionRepository, options);
