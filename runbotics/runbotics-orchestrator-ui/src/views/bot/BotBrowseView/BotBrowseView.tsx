import React, { ChangeEvent, useCallback, useMemo, VFC } from 'react';
...
const BotBrowseView: VFC = () => {
    ...
    const hasAccessToBotsCollectionTab = useFeatureKey([FeatureKey.BOT_COLLECTION_READ]);
    const hasAccessToBotsTab = useFeatureKey([FeatureKey.BOT_READ]);

    const botTabs = useMemo(() => {
        const tabs = [];

        if (hasAccessToBotsTab) {
            tabs.push({ value: BotCollectionTab.BOTS, label: translate('Bot.Browse.Tabs.Bots.Label') });
        }

        if (hasAccessToBotsCollectionTab) {
            tabs.push({ value: BotCollectionTab.COLLECTIONS, label: translate('Bot.Browse.Tabs.Collections.Label') });
        }

        return tabs;
    }, [hasAccessToBotsTab, hasAccessToBotsCollectionTab]);
    ...
    return (
        <StyledPage title={getPageTitle()}>
            <StyledContainer maxWidth={false}>
                <Box>
                    <If condition={hasAccessToBotsCollectionTab || hasAccessToBotsTab}>
                        <Tabs
                ...
                <If 
                    condition={tab === BotCollectionTab.COLLECTIONS && hasAccessToBotsCollectionTab}
                    else={
